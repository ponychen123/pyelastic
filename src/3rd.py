#!/usr/bin/python3
#this module calculate the third order elastic constants
#author:ponychen
...
f.close()
mode = int(inputfile[40].split()[0])  #1 for create calculation input files and 2 for calculate modulus
max_strain = float(inputfile[41].split()[0])  #the max applied strain used, carefully choose
numbers = int(inputfile[42].split()[0])  #the number of sampling points
input_type = inputfile[43].split()[0]  #"qe" "vasp" "elk"or "kkr", as far support these three
bravi_type = inputfile[44].split()[0] #"cubic3rd"
kkr_atoms = int(inputfile[45].split()[0])  #if you set input_type equal to kkr, the kkr_atoms means the number of total atoms
...
if mode == 1:
    ...
    parameters = analy.getparameter3(results,types,numbers,max_strain,axis,bravi_type)
    #get the modulus by parameters
    modulus = analy.getmodulus(parameters,bravi_type)
    #output the final result
    iio.finalout3rd(modulus)
else:
